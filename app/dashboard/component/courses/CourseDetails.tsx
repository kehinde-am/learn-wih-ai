  ...
  return (
    <div className="fixed inset-0 z-50 bg-gray-800 bg-opacity-50 flex justify-center items-center">
      <div className="bg-white p-6 rounded-lg shadow-lg max-w-lg w-full max-h-[80vh] overflow-y-auto relative">
        <h2 className="text-2xl font-bold mb-4">AI Feedback on Your Performance</h2>
...
const CourseDetails: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (user) {
      ...
      if (selectedCourseId) {
        fetchProgress(user.email ?? "", selectedCourseId);
      }
    }
  ...
  const generateAiFeedback = async (courseName: string): Promise<string> => {
    ...
    console.log("Sending AI feedback request with prompt:", prompt);
    ...
    try {
      ...
      console.log("Received AI feedback:", feedbackData);
